@echo off
REM agentchattr - starts server (if not running) + Grok Build CLI with auto-approve
cd /d "%~dp0.."
...
REM Pre-flight: check that Grok Build CLI is installed
where grok >nul 2>&1
if %errorlevel% neq 0 (
    echo.
    echo   Error: "grok" was not found on PATH.
    echo   Install Grok Build CLI first, then try again.
    echo.
    pause
    exit /b 1
)
...
python wrapper.py grok --always-approve
if %errorlevel% neq 0 (
    echo.
