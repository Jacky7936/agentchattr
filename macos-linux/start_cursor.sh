#!/usr/bin/env sh
# agentchattr - starts server (if not running) + Cursor Builder wrapper
cd "$(dirname "$0")/.."
...
.venv/bin/python wrapper.py cursor --profile cursor-builder --model composer-2.5-fast --yolo --sandbox disabled --approve-mcps
