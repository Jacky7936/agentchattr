#!/usr/bin/env sh
# agentchattr - starts server (if not running) + Antigravity CLI with auto-approve
cd "$(dirname "$0")/.."
...
if ! command -v agy >/dev/null 2>&1; then
    ...
    echo "  Mac/Linux: curl -fsSL https://antigravity.google/cli/install.sh | bash"
    echo ""
    exit 1
fi
...
.venv/bin/python wrapper.py antigravity --dangerously-skip-permissions
