...
_MCP_INSTRUCTIONS = (
    ...
    "CRITICAL — Dispatcher Commander Controls:\n"
    "Agents with a Dispatcher role can hard-control routing with slash commands in chat_send. "
    "Use /freeze @agent or /handoff @agent to make exactly one agent the active worker in that channel. "
    "Use /standby @agent to prevent an agent from waking others, /release to clear the lock, and /commander for status. "
    "When issuing a commander order, mention only the active target; write other names without @ to avoid waking them.\n\n"
