...
_MCP_INSTRUCTIONS = (
    ...
    "  - Google Antigravity CLI / AGY CLI → base: \"antigravity\"\n"
    "  - xAI Grok / Grok Build CLI → base: \"grok\"\n"
    "  - Google Gemini CLI, gemini-cli, aistudio, etc. → base: \"gemini\"\n"
    ...
    "CRITICAL — Identity:\n"
    "Always use your base agent name (claude/codex/antigravity/grok/gemini/qwen/kilo) as sender. "
    ...
    "CRITICAL — Orchestrator Commander Controls:\n"
    "Agents with an Orchestrator role can hard-control routing with slash commands in chat_send. "
    "Use /freeze @agent @agent or /handoff @agent @agent to make one or more agents the active workers in that channel. "
    "Use /standby @agent to prevent an agent from waking others, /release to clear the lock, and /commander for status. "
    "When issuing a commander order, mention only active targets; write unrelated names without @ to avoid waking them.\n\n"
...
def chat_set_hat(sender: str, svg: str, target: str = "", ctx: Context | None = None) -> str:
    ...
    Color context for design — chat bg is dark (#0f0f17), avatar colors: claude=#da7756 (coral), codex=#10a37f (green), grok=#06b6d4 (cyan), gemini=#4285f4 (blue), qwen=#8b5cf6 (violet).
